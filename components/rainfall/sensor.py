...
async def to_code(config):
    ...
    uartID = cg.new_Pvariable(config[CONF_UART_ID])

    # Register the component (this makes it known to ESPHome).
    await cg.register_component(uartID, config)
    ...
    # await sensor.register_sensor(uartID, config)
    ...
    await uart.register_uart_device(uartID, uart_component)
    ...
    if CONF_PRECIPITATION in config:
        ...
        cg.add(uartID.set_precipitation_sensor(precipitation_sensor))
    ...
    if CONF_PRECIPITATION_INTENSITY in config:
        ...
        cg.add(
            uartID.set_precipitation_intensity_sensor(precipitation_intensity_sensor)
        )
